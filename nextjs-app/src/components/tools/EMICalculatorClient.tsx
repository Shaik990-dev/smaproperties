      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-3 bg-white rounded-2xl p-6 lg:p-8 shadow-sm border border-gray-100">
            ...
            <Slider label="Loan Amount" value={principal} onChange={setPrincipal} min={100000} max={50000000} step={50000} format={formatINR} quickValues={[1000000, 2500000, 5000000, 10000000]} quickFormat={(n) => (n >= 10000000 ? `₹${n / 10000000}Cr` : `₹${n / 100000}L`)} />
            <Slider label="Interest Rate" value={rate} onChange={setRate} min={1} max={20} step={0.05} format={(n) => `${n.toFixed(2)}% per annum`} quickValues={[7.5, 8.5, 9.5, 10.5]} quickFormat={(n) => `${n}%`} />
            <Slider label="Loan Tenure" value={years} onChange={setYears} min={1} max={30} step={1} format={(n) => `${n} year${n === 1 ? '' : 's'}`} quickValues={[5, 10, 15, 20, 25, 30]} quickFormat={(n) => `${n}y`} />
          </div>
          ...
          <aside className="lg:col-span-2 space-y-4">
            <div className="bg-gradient-to-br from-[var(--color-navy)] to-[var(--color-navy-light)] text-white rounded-2xl p-6 shadow-xl">
              ...
              <div className="font-display text-4xl font-black text-[var(--color-amber-light)] mt-2">{formatINR(emi)}</div>
              ...
              <div className="mt-5">
                <div className="text-xs text-white/60 mb-2">Interest is <strong className="text-[var(--color-amber-light)]">{interestPercent.toFixed(1)}%</strong> of total payment</div>
...
interface SliderProps {
  ...
  format: (v: number) => string; quickValues: number[]; quickFormat: (v: number) => string;
}
...
function Slider({ label, value, onChange, min, max, step, format, quickValues, quickFormat }: SliderProps) {
  return (
    <div className="mb-7">
      ...
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full h-2 rounded-full bg-gray-200 appearance-none cursor-pointer accent-[var(--color-navy)]" />
      <div className="flex flex-wrap gap-1.5 mt-3">
        {quickValues.map((v) => (
          <button key={v} onClick={() => onChange(v)} className={`px-3 py-1 rounded-md text-xs font-bold border ${Math.abs(value - v) < step * 0.5 ? 'bg-[var(--color-navy)] text-white border-[var(--color-navy)]' : 'bg-gray-50 text-gray-600 border-gray-200 hover:border-[var(--color-navy)]'}`}>
            {quickFormat(v)}
          </button>
        ))}
      </div>
